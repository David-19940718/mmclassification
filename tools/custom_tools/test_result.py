...
TOTAL_CLASS = total_class = [item for sublist in CLASSES for item in sublist]
...
def save_error(error_info, save_path, save_label=False):
...
def main():
    ...
    parser.add_argument('--save-error', action='store_true', 
                        help='Saving error samples.')
    parser.add_argument(
        '--device', default='cuda:0', help='Device used for inference')
    args = parser.parse_args()
    
    pred, target, pd, gt, error_info = get_inds(args.GT, args.PD)
    ...
    if args.save_error: 
        save_error(error_info, args.PD, save_label=True)
