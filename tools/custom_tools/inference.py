...
def inference_multi_task_model(model, img, CLASSES, mode='eval'):
    """Inference image(s) with the multi-task classifier.
    Args:
        model (nn.Module): The loaded classifier.
        img (str/ndarray): The image filename or loaded image.
        CLASSES： [['black', 'blue', 'yellow'],
                   ['bus', 'car', 'suv']]
    Returns:
        result (dict): The classification results that contains
            `class_name`, `pred_label`, and `pred_score`.
    ...
    with torch.no_grad():
        ...
        topk_pred_score, topk_pred_index, topk_pred_class = [], [], []
        ...
        for c in range(len(CLASSES)):
            ...
            if mode == 'eval':
                pred_label = CLASSES[c][pred_index]
            elif mode == 'tag':
                pred_label = CLASSES[c][pred_index] if pred_score > 0.5 else 'others'
            topk_pred_score.append(pred_score)
            topk_pred_index.append(pred_index)
            topk_pred_class.append(pred_label)
