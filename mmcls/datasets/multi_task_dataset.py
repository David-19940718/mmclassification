...
class Multi_Task_Dataset(MultiLabelDataset):
    CLASSES = [
        [
        'black',
        'blue',
    ...
    def load_annotations(self):
        ...
        for img_id in img_ids:
            ...
            indexs = [[self.CLASSES[i].index(label[0])] for i, label in enumerate(labels)]
            gt_label = [np.zeros(len(self.CLASSES[n])) for n in range(len(self.ATTRIBUTES))]
            for i in range(len(self.ATTRIBUTES)):
                gt_label[i][indexs[i]] = 1
                gt_label[i].astype(np.int8)
            ...
            For example:
                ...
                info = {
                    ...
                    'gt_label': [array([0., 0., 0., 1., 0., 0., 0., 0., 0.]), array([0., 0., 0., 1., 0.])], dtype=int8)
                }
            '''
